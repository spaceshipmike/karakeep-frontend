 ...
 */
export function BookmarkCardCompact({ bookmark, onEdit }: BookmarkCardCompactProps) {
  const screenshotUrl = getBookmarkScreenshotUrl(bookmark);
  const title = getBookmarkTitle(bookmark);
  const url = bookmark.content?.url;
  const favicon = bookmark.content?.favicon;
    ...
    <article className="group relative flex gap-4 rounded-lg border border-border bg-card p-4 transition-all duration-200 hover:border-primary/30 hover:bg-card/80">
      ...
      {onEdit && (
        <button
          ...
          className="absolute right-3 top-1/2 z-10 -translate-y-1/2 rounded-md p-2 text-muted-foreground opacity-0 transition-all hover:bg-muted hover:text-foreground group-hover:opacity-100"
          title="Edit bookmark"
